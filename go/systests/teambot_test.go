...
func noNewTeambotKeyNotification(tc *libkb.TestContext, notifications *teamNotifyHandler) {
	select {
	case <-notifications.newTeambotKeyCh:
		require.Fail(tc.T, "unexpected newTeambotKey notification")
	default:
	}
}
...
func noTeambotKeyNeeded(tc *libkb.TestContext, notifications *teamNotifyHandler) {
	select {
	case <-notifications.teambotKeyNeededCh:
		require.Fail(tc.T, "unexpected teambotKeyNeeded notification")
	default:
	}
}
...
func TestTeambotKey(t *testing.T) {
	...
	require.Error(t, err)
	require.IsType(t, teambot.TeambotKeyError{}, err)
	...
	newKeyArg = keybase1.NewTeambotKeyArg{
		Id:         teamID,
		Generation: teambotKey4.Metadata.Generation + 2,
	}
	checkNewTeambotKeyNotifications(botua.tc, botua.notifications, newKeyArg)
	...
	badGen := teambotKey.Metadata.Generation + 1
